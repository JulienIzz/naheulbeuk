...
type DefaultFailOnConsoleOptions = {
  consoleMessagesToIgnore: RegExp[];
};

const defaultFailOnConsoleOptions: DefaultFailOnConsoleOptions = {
  consoleMessagesToIgnore: [],
};

export const setUpFailOnConsole = ({
  consoleMessagesToIgnore,
} = defaultFailOnConsoleOptions) => {
  failOnConsole({
    shouldFailOnWarn: true,
    shouldFailOnDebug: true,
    silenceMessage: (consoleMessage: string) => {
      if (
        consoleMessagesToIgnore.some((consoleMessageToIgnore) =>
          consoleMessageToIgnore.test(consoleMessage),
        )
      ) {
        return true;
      }
